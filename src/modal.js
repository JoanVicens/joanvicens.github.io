...
function projectfullScreen(modal, img_index) {
    ...
    // Copies the carousel
    let glide = project.glide.cloneNode(true)
    ...
    imageViewComponent.content_wrapper.appendChild(project.content.cloneNode(true));
...
export function showFullDescription(hash) {
    ...
    if(modal) {
        ...
        // Carousel
        let carousel = modal.querySelector('.glide')
        let images_carousel = carousel.querySelectorAll('img');
        ...
        Array.from(images_carousel)
            .forEach(image => image.onclick = (event) => {
                let image_clicked = Array.from(images_carousel).indexOf(event.target);
